    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                ...
                if cfg.MODEL.RE:
                    ...
                    for i in range(0, index, 2):
                        ...
                        loss = loss + loss_tmp
                    if cfg.DATASETS.NAMES == 'market_to_RGBNT201':
                        loss = loss + output[8]+output[9]
                    else:
                        loss = loss + output[2]+output[3]
                    # loss_33 = loss_fn(score=output[7], feat=output[8], target=target, target_cam=target_cam)
                else:
                    index = len(output)-1
                    for i in range(0, index, 2):
                        loss_tmp = loss_fn(score=output[i], feat=output[i + 1], target=target, target_cam=target_cam)
                        loss = loss + loss_tmp +output[-1]
            ...
            if isinstance(output, list):
                acctotal = (output[0][0].max(1)[1] == target).float().mean()
            else:
                acctotal = (output[0].max(1)[1] == target).float().mean()



            loss_meter.update(loss.item(), img['RGB'].shape[0])
            acc_meter.update(acctotal, 1)


            torch.cuda.synchronize()
            if (n_iter + 1) % log_period == 0:
                # print(scheduler._get_lr(epoch))
                logger.info("Epoch[{}] Iteration[{}/{}] Loss: {:.3f},Losscon: {:.3f},acctotal: {:.3f}, Base Lr: {:.2e}"
                            .format(epoch, (n_iter + 1), len(train_loader),
                                    loss_meter.avg,output[-1].item(),acc_meter.avg, scheduler._get_lr(epoch)[0]))
