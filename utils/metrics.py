...
class R1_mAP():
    ...
    def update(self, output):
        ...
        cpu_tensor = sceneid.cpu()

# 然后将 CPU 上的张量转换为 NumPy 数组
        self.sceneids.extend(cpu_tensor.numpy())
    ...
    def compute(self):
        ...
        # np.save('visualization/features/1/qfeatures.npy',qf.cpu().numpy())
        # np.save('visualization/ids/qpids.npy',q_pids)
        ...
        # np.save('visualization/features/1/gfeatures.npy',gf.cpu().numpy())
        # np.save('visualization/ids/gpids.npy',g_pids)
...
class R1_mAP_eval():
    ...
    def compute(self):  # called after each epoch
        ...
        q_pids = np.asarray(self.pids[:self.num_query])
        q_camids = np.asarray(self.camids[:self.num_query])
        # np.save('visualization/person/features/3/qfeatures.npy',qf.cpu().numpy())
        # np.save('visualization/person/ids/qpids.npy',q_pids)
        # print("save query done!!!!!!!!!!!")
        # gallery
        gf = feats[self.num_query:]
        g_pids = np.asarray(self.pids[self.num_query:])
        ...
        g_camids = np.asarray(self.camids[self.num_query:])
        # np.save('visualization/person/features/3/gfeatures.npy',gf.cpu().numpy())
        # np.save('visualization/person/ids/gpids.npy',g_pids)
        # print("save gallery done!!!!!!!!!!")
        result = {'gallery_f':gf.numpy(),'gallery_label':g_pids,
                'gallery_cam':g_camids,'query_f':qf.numpy(),
                'query_label':q_pids,'query_cam':q_camids}
        scipy.io.savemat('/media/yangxilab/f757d9ef-e333-4bad-8bf1-32aeaaf2f3ca/dwj/TOP-ReID-master-copy2/visualization/'+'.mat',result)
        print("sane done!!!!!!!!!!!!!!!!!!!!")
