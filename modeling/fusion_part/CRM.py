...
from layers.focal_frequency_loss import FocalFrequencyLoss
import torch
from layers.reconstruction import CosineSimilarityLoss, FeatureKLLoss, ContrastiveFeatureLoss
...
class CRM(nn.Module):
    ...
    def __init__(self, dim, num_heads, depth=1, miss='nothing'):
        ...
        print("CRM HERE!!!")
        self.miss = miss
        self.criterion_f = FocalFrequencyLoss()
        # # self.criterion_f = CosineSimilarityLoss(reduction='mean')
        # # self.criterion_f = FeatureKLLoss(reduction='sum')
        # self.criterion_f = ContrastiveFeatureLoss(margin=0.5, reduction='sum')
    ...
    def forward(self, ma, mb, mc,flag,miss=None):
        if self.training and flag:
            RGB_NI, RGB_TI = self.RGBRE(ma)
            NI_RGB, NI_TI = self.NIRE(mb)
            TI_RGB, TI_NI = self.TIRE(mc)
          
            loss_rgb = nn.MSELoss()(RGB_NI, mb) + nn.MSELoss()(RGB_TI, mc) + 0.05*self.criterion_f(RGB_NI[:, 0, :], mb[:, 0, :])+0.05*self.criterion_f(RGB_TI[:, 0, :], mc[:, 0, :])
            loss_ni = nn.MSELoss()(NI_RGB, ma) + nn.MSELoss()(NI_TI, mc)+0.05*self.criterion_f(NI_RGB[:, 0, :], ma[:, 0, :]) +0.05*self.criterion_f(NI_TI[:, 0, :], mc[:, 0, :])
            loss_ti = nn.MSELoss()(TI_RGB, ma) + nn.MSELoss()(TI_NI, mb)+0.05*self.criterion_f(TI_RGB[:, 0, :], ma[:, 0, :]) +0.05*self.criterion_f(TI_NI[:, 0, :], mb[:, 0, :])
            ...
            # loss2 = discriminator_loss
            loss1 = loss_rgb + loss_ni + loss_ti
            # print("loss",loss)
            return loss1
        else:
            if miss == None:
